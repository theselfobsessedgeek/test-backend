...
class UniversityCourses:
    ...
    def __init__(self, rank=0) -> None:
        ...
        self.nonDeutsch_unicors = [] # {'id' : 0, 'university' : None, 'courses' : []}
    ...
    def find_and_save_non_Deutsch(self):
        
        Deutsch_count = 1
        non_Deutsch = 1
        None_count = 1
        for unco in self.unicors:
            courses = []
            for cor in unco['courses']:
                if not cor['Hauptunterrichtssprache']:
                    None_count = None_count + 1 #pass #print(cor)
                    courses.append(cor)
                else:
                    if 'Deutsch' in cor['Hauptunterrichtssprache']:
                        Deutsch_count = Deutsch_count + 1
                    else:
                        non_Deutsch = non_Deutsch + 1 #print(cor)
                        courses.append(cor)
            if len(courses) > 0:
                self.nonDeutsch_unicors.append({'id' : unco['id'], 'university' : unco['university'], 'courses' : courses})
        with open('find_and_save_non_Deutsch' + '.json','w') as fp:
                    json.dump(self.nonDeutsch_unicors, fp)
        if ENABLE_PRINTS:
            print(f"Courses count in Decutsch : {Deutsch_count}")
            print(f"Courses count in non Decutsch : {Deutsch_count}")
            print(f"Courses count in Uuknown Language : {None_count}")
            # print(cor)
            # input('cor')
                # if cor is not None and cor['Hauptunterrichtssprache'] is not None:
                #     if 'Deutsch' not in cor['Hauptunterrichtssprache']:
                #         Course().setCourse(cor).printcourse('\t')
    def fienamewithfolder(self, id_rank:int):
        if not os.path.isdir(UNIV_DIR):
            os.mkdir(UNIV_DIR)
